...
- **Email:** ~~the sender is still the dev-only logger~~ **superseded 2026-09-03** — production now sends through Brevo (`BrevoEmailSender`); see [`../auth/password-reset.md`](../auth/password-reset.md) §7. The original note read: the sender is the dev-only logger (`LoggingEmailSender`), so verification emails
