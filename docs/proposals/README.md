...
| [`quiz-view-redesign.md`](./quiz-view-redesign.md) | Should the single-quiz page drop its three tabs for one continuous page built around per-question performance? | Two decisions: who may open the page (it is admin-gated, its analytics are owner-only), and whether the analytics DTO grows. |
