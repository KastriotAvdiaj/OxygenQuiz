...
Status: **slice 2.0 backend, branch `feat/ai-quiz-generation`.** Topic and source-text modes,
plain JSON response, quota, spend cap, kill switch. No streaming, no file upload, no frontend
yet. Last updated: 2026-08-09
...
show "4 of 5 left" and hide the Generate button when the feature is off, rather than offering a
...
| 3 | `DefaultDailyQuota` = 5 | One user to ~5 generations/day | `AiQuotaService` |
...
**2. The prompt exists twice.** `AiPromptBuilder.cs` and `prompt.ts` are now duplicates, and
nothing enforces that they agree. This is exactly the drift the plan warned about; it is
deliberate only because slice 2.1 deletes one of them. **If 2.1 slips, this becomes the most
likely source of a subtle bug in the feature** — a prompt tweak that improves generated quizzes
and silently doesn't apply to copy-paste ones.
...
**5. Rate limiting partitions by IP, quota by user.** A classroom or office behind one NAT shares
the 6/min burst limit — plausibly our actual audience. If it bites, the `ai` policy should
partition on user id for authenticated callers instead of IP.
...
**10. `ai-prompt` shares the `ai` rate-limit policy.** Copying a prompt — which is free and
should be encouraged — eats the same 6/min budget as generating. Give it the global limiter, or
its own looser policy, before the copy-paste path starts calling it in 2.1.
