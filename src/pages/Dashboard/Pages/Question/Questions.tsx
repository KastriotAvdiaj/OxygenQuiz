...
export const Questions = () => {
  ...
  const [debouncedSearchTerm, setDebouncedSearchTerm] = useDebounce(searchTerm, 500); // Increased debounce
  const [selectedCategoryId, setSelectedCategoryId] = useState<number | undefined>();
  const [selectedDifficultyId, setSelectedDifficultyId] = useState<number | undefined>();
  ...
  const { isOpen: isAddQuestionDialogOpen, open: openAddQuestionDialog, close: closeAddQuestionDialog } = useDisclosure();
  ...
  useEffect(() => {
    setPageNumber(1);
  }, [debouncedSearchTerm, selectedCategoryId, selectedDifficultyId, selectedLanguageId]);
  ...
  if (mcqQuery.isError) {
    return <p className="text-center text-red-500 py-8">Failed to load questions. Please try again later.</p>;
  }

  return (
    <div className="container mx-auto py-8 px-4 md:px-0"> {/* Added container for better layout */}
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Questions Management</h1>
        {/* "Add Question" Dialog Trigger */}
        <Dialog open={isAddQuestionDialogOpen} onOpenChange={(open) => (open ? openAddQuestionDialog() : closeAddQuestionDialog())}>
      ...
      <Card className="p-6 bg-card border"> {/* Changed from bg-background */}
        <QuestionFilters
